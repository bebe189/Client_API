...
public class CommentController {
    ...
    @CrossOrigin(allowedHeaders = "*", origins = "*")
    public ResponseEntity<jsonResponseBody> deleteComment(@PathVariable(value = "id") long id) {
        Optional<Comment> c = commentService.getCommentById(id);
        if(c.isPresent()){
            ...
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body(new jsonResponseBody(HttpStatus.NO_CONTENT.value(), "Non trovato"));
        }
    }
    @AllArgsConstructor
