...
public class BlogUserController {
    ...
    @CrossOrigin(allowedHeaders = "*", origins = "*")
    public ResponseEntity<jsonResponseBody> deleteUserById(@PathVariable(value = "id") Long id) {
        Optional<BlogUser> b = blogUserservice.getUserByID(id);
        if (b.isPresent()) {
            blogUserservice.deleteUserService(id);
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body(new jsonResponseBody(HttpStatus.NO_CONTENT.value(), null));
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new jsonResponseBody(HttpStatus.NOT_FOUND.value(), "non trovato.."));
        }
    }
